...
class _TranslatorScreenState extends State<TranslatorScreen> {
  ...
  Future<void> _translate() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          _translatedText = data['responseData']['translatedText'] ?? 'No translation received';
        });
        // Optional auto speak
        // await _speak();
      } else {
  ...
  List<DropdownMenuItem<String>> _buildLangItems() {
    return _languages.keys.map((name) => DropdownMenuItem(
      value: name,
      child: Text(name),
    )).toList();
  }
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            ...
            if (_translatedText.isNotEmpty) ...[
              const Text('Traducción:', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
