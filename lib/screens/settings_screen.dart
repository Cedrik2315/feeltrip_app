...
class _SettingsScreenState extends ConsumerState<SettingsScreen> {
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildAmbassadorTile(context, ref),
  ...
  Widget _buildAmbassadorTile(BuildContext context, WidgetRef ref) {
      ...
      data: (sub) {
        if (sub.isAmbassador) {
          return _settingsTile(
            ...
            subtitle: 'Saldo Viaje: \$${sub.ambassadorBalance.toStringAsFixed(2)} | Suscripciones: ${sub.ambassadorSalesCount}',
            iconShadows: [
              Shadow(color: Theme.of(context).colorScheme.primary.withValues(alpha: 0.5), blurRadius: 10),
            ],
            onTap: () {
              // Navegar al HUB de Embajador (Próxima actualización)
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('Accediendo al Hub de Embajador...')),
              );
            },
          );
        } else {
          return _settingsTile(
            ...
            title: 'CONVERTIRSE EN EMBAJADOR',
            subtitle: 'Viaja gratis vendiendo experiencias',
            onTap: () => _showAmbassadorInfo(context),
          );
        }
      },
  ...
  void _showAmbassadorInfo(BuildContext context) {
    final colorScheme = Theme.of(context).colorScheme;
      ...
      builder: (context) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'No es un trabajo, es un estilo de vida. Como Embajador, viajas por el mundo utilizando y documentando tus experiencias en FeelTrip.',
            ...
            const SizedBox(height: 24),
            _buildBenefitRow(context, Icons.flight_takeoff, 'Viajes costeados por la comunidad.'),
            _buildBenefitRow(context, Icons.monetization_on_outlined, 'Comisión directa por cada suscripción vendida.'),
            _buildBenefitRow(context, Icons.map_outlined, 'Acceso anticipado a rutas exclusivas.'),
            const SizedBox(height: 32),
            Text(
              'TU ÚNICA MISIÓN: Vender tu experiencia. Una parte de cada suscripción que generes se acumula en tu "Travel Wallet" para costear tus próximos pasajes.',
              style: GoogleFonts.jetBrainsMono(
                fontSize: 11,
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed: () async {
                  ...
                  if (userId != null) {
                    await ref.read(subscriptionServiceProvider).setAmbassadorStatus(
                      userId, 
                      isActive: true,
                      initialBalance: 50.0, // Bono de bienvenida para testeo
                    );
                    if (context.mounted) {
                      Navigator.pop(context);
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text('SISTEMA ACTUALIZADO: Rango EMBAJADOR activado con éxito.'),
                          backgroundColor: Color(0xFF00FF41), // Terminal Green
                        ),
                      );
                    }
                  }
